...
OutputDir=.\Output
OutputBaseFilename=FHUORs2_RU_v.2.0.1
DisableDirPage=no
...
[Messages]
WelcomeLabel2=Установка русской локализации для Five Hearts Under One Roof season 2 (v.2.0.1) с MelonLoader
...
[Components]
Name: translation; Description: "Русская локализация (v.2.0.1)"; Types: full; Flags: fixed
...
[Icons]
Name: "{group}\FHUORs2-RU (v.2.0.1)"; Filename: "{app}\FHUOR2.exe"; IconFilename: "{app}\icon.ico"; Comment: "Five Hearts Under One Roof season 2 with Russian Localisation"
Name: "{group}\Удалить русскую локализацию"; Filename: "{uninstallexe}"
Name: "{commondesktop}\FHUORs2 (v.2.0.1)"; Filename: "{app}\FHUOR2.exe"; IconFilename: "{app}\icon.ico"; Comment: "Запустить с русской локализацией"
...
begin
  ChangelogPage := CreateCustomPage(wpWelcome, 'Изменения в версии 2.0.1', 
  ...
  ChangelogMemo.Text := 
    ...
    '• We''re going to test the S/S outfit samples,=Мы будем примерять коллекцию одежды весна-лето,' + #13#10 +
    '• How''s the upcoming S/S collection?=Какие мысли предстоящей коллекции весна-лето?' + #13#10 +
    '• Is this the new S/S collection?=Ну как новая коллекция весна-лето?' + #13#10 +
    '• Chapter 4: Try on all of Gran''s S/S collection=Глава 4: Примерить всю коллекцию весна-лето Гран' + #13#10 +
    '• I told you—we just stopped by because someone got hurt!=Я же сказал — мы просто зашли, потому что Гюри пострадала!' + #13#10 +
    '• Don''t lean too much on the bar—like this.=Не опирайся слишком сильно на поручни — вот так.' + #13#10 +
    '• Here, your glute muscles.=Ну, мои ягодичные мышцы.' + #13#10 +
    '• Can you feel the muscles properly?=Мышцы напрягаются как нужно?' + #13#10 +
    '• Group Photo: School days=Групповое фото: Школьные дни' + #13#10 +
    '• Group Photo: Beach=Групповое фото: Пляж' + #13#10 +
    '• Group Photo: Walking path=Групповое фото: Зеленая тропа' + #13#10#13#10 +
    'Версия 2.0:' + #13#10 +
    '===========' + #13#10#13#10 +
    'Глобальные изменения:' + #13#10 +
...
begin
  ...
  begin
    MsgBox('Русская локализация FHUORs2 (v.2.0.1) успешно установлена!' + #13#10#13#10 +
